...
function custom_star_rating( $args = array() ) {
	$star_base_class       = 'star';
	$star_active_classes   = "$star_base_class active";
	$star_symbol_id        = 'star';
	$star_half_gradient_id = 'half'
	...
	>
		<defs>
			<linearGradient id="<?php echo esc_attr( $star_half_gradient_id ); ?>" x1="0" x2="100%" y1="0" y2="0">
				<stop offset="50%" class="<?php echo esc_attr( $star_active_classes ); ?>"></stop>
				<stop offset="50%" class="<?php echo esc_attr( $star_base_class ); ?>"></stop>
			</linearGradient>

			<symbol xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" id="<?php echo esc_attr( $star_symbol_id ); ?>">
				<path
	...
	$empty_star_el = '
		<svg 
			class="' . $star_base_class . ' ' . $star_classes . '"
			width="12" 
			height="12" 
			viewBox="0 0 24 24"
		>
			<use xlink:href="#' . $star_symbol_id . '"></use>
		</svg>
	';

	$full_star_el = '
		<svg 
			class="' . $star_active_classes . ' ' . $star_classes . '"
			width="12" 
			height="12" 
			viewBox="0 0 24 24"
		>
	...
	$half_star_el = '
		<svg 
			...
			viewBox="0 0 24 24"
		>
			<use xlink:href="#' . $star_symbol_id . '" fill="url(#' . $star_half_gradient_id . ')"></use>
		</svg>
	';

	$wrapper_classes = $parsed_args['wrapper_classes'];

	$stars_number = $full_stars;

	if ( $half_stars > 0 ) {
		$stars_number += 0.5;
	}

	$output  = '<div class="flex ' . $wrapper_classes . '" aria-label="' . $stars_number . ' stars out of ' . $rating_stars_number . '">';
