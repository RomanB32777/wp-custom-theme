...
if ( ! class_exists( 'Header_Walker_Nav_Menu' ) ) {
	...
	class Header_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		function start_el( &$output, $data_object, $depth = 0, $args = null, $current_object_id = 0 ) {
			...
			$link_classes = array(
				$depth > 0 ? 'sub-menu-link block no-underline rounded-lg py-2 pl-6 pr-3 w-full duration-200 md:!py-1 md:!px-0' : 'main-menu-link no-underline block px-3 py-2 rounded-lg duration-200 md:!rounded-none md:!px-0',
			);
			...
			if ( isset( $args->has_children ) && $args->has_children && $args->depth > 1 ) {
				...
				$link_classes[] = 'w-full !flex items-center gap-x-1 text-base font-bold duration-200 pl-3 pr-3.5 md:!font-normal md:!px-0 md:!w-auto';
			}
