...
function scripts_theme() {
	...
	enqueue_theme_versioned_script( 'theme-swiper', '/dist/js/swiper.js' );
	enqueue_theme_versioned_script( 'wp_custom_main_script', '/dist/js/main.js', array( 'jquery', 'theme-swiper' ) );
...
function theme_widgets_init() {
	register_sidebar(
		array(
			...
			'id'            => 'right-sidebar',
			'description'   => esc_html__( 'Add widgets here so that it appears on the sidebar.', 'custom-theme' ),
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Bottom content', 'custom-theme' ),
			'id'            => 'bottom-widgets',
			'description'   => esc_html__( 'Add widgets here so that it appears on the page bottom.', 'custom-theme' ),
			'before_widget' => '<div id="%1$s" class="%2$s">',
			'after_widget'  => '</div>',
			'before_title'  => '',
			'after_title'   => '',
...
function set_approver_on_transition_only( $new_status, $old_status, $post ) {
	if ( $old_status == $new_status || $old_status != 'pending' || $new_status != 'publish' ) {
		return;
	}
